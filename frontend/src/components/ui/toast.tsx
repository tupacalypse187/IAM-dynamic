...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-6 shadow-lg transition-all data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-top-2 data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
...
// Radix requires ToastViewport (and Toast roots) to be inside its own provider
// (distinct from the app-level ToastProvider context in use-toast.tsx)
const RadixToastProvider = ToastPrimitives.Provider

export { Toast, ToastTitle, ToastDescription, ToastClose, ToastViewport, RadixToastProvider }
