...
class PolicyResponse:
    ...
    Attributes:
        ...
        suggested_refinement: Suggested better request (for rejection guidance)
    """
    def __init__(self, policy: Dict[str, Any], risk: str, explanation: str, approver_note: str, suggested_refinement: str = ""):
        self.policy = policy
        self.risk = risk
...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self):
        ...
        # GPT-5.1 (latest) - GPT-5 is previous model
        # Model code: gpt-5.1 (OpenAI recommends using latest)
        self.model_name = os.getenv("OPENAI_MODEL", "gpt-5.1")
        if self.api_key:
...
- approver_note: recommendation for the approver
- suggested_refinement: suggested better request (optional)"""
...
def validate_provider(provider_type: str) -> tuple[bool, str]:
    """
    Check if provider is properly configured with API key and dependencies
    ...
    if provider_type == "gemini":
        ...
        try:
            import google.generativeai as genai
            return True, "Gemini provider is ready"
        except ImportError:
        ...
        try:
            import openai
            return True, "OpenAI provider is ready"
        except ImportError:
        ...
        try:
            import anthropic
            return True, "Anthropic provider is ready"
        except ImportError:
        ...
        if not api_key:
            return False, "ZHIPUAI_API_KEY not configured"
        if not ZHIPUAI_AVAILABLE:
            return False, "zhipuai package not installed. Run: pip install zhipuai"
        return True, "Zhipu provider is ready"
